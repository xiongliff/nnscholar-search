    ...
    try:
        ...
        with open (jcr_file, 'r', encoding='utf-8') as f:
            try:
                ...
                logger.info (
                    f"成功加载期刊数据，包含 {len (journal_list)} 条记录"
                    )
                ...
                if len (journal_list) > 0:
                    ...
                    logger.info (
                        f"原始数据示例: {json.dumps (sample_raw, ensure_ascii=False)}"
                        )
                ...
                for journal in journal_list:
                    ...
                    if issns:
                        ...
                        try:
                            ...
                            impact_factor = 'N/A'
                            logger.warning (
                                f"无效的影响因子值: {journal.get ('IF')} for {journal.get ('journal')}"
                                )
                ...
                # 记录一些转换后的数据示例
                if journal_data:
                    sample_converted = {k: journal_data[k] for k in
                                        list (journal_data.keys ())[:3]}
                    logger.info (
                        f"转换后的数据示例: {json.dumps (sample_converted, ensure_ascii=False)}"
                        )
            ...
            with open (trend_file, 'r', encoding='utf-8') as f:
                try:
                    ...
                    if not isinstance (if_trend_data, dict):
                        ...
                        logger.info (
                            f"成功加载影响因子趋势数据，包含 {len (if_trend_data)} 条记录"
                            )
                except json.JSONDecodeError as e:
...
def get_journal_metrics (issn):
    ...
    try:
        ...
        logger.info (
            f"获取到的原始期刊信息: {json.dumps (journal_info, ensure_ascii=False)}"
            )
        ...
        logger.info (
            f"处理后的期刊指标: {json.dumps (metrics, ensure_ascii=False)}"
            )
        return metrics
...
def filter_papers_by_metrics (papers, filters):
    ...
    try:
        ...
        if 'year_range' in filters and filters['year_range']:
            ...
            for paper in papers:
                ...
                try:
                    ...
                    logger.debug (
                        f"文献 {paper.get ('pmid')} 发表年份: {pub_year}"
                        )
                    if pub_year and year_range[0] <= pub_year <= year_range[1]:
                        ...
                        logger.debug (
                            f"文献 {paper.get ('pmid')} 未通过年份筛选: {pub_year} not in {year_range}"
                            )
                except (ValueError, TypeError) as e:
                    logger.warning (
                        f"年份格式错误: {pub_year}, 错误信息: {str (e)}"
                        )
        else:
        ...
        logger.info (
            f"1. 年份筛选 ({filters.get ('year_range', '无限制')}): {len (papers)} -> {len (year_filtered)}"
            )
        ...
        if len (year_filtered) == 0:
            ...
            for paper in papers:
                ...
                logger.warning (
                    f"PMID: {pmid}, 年份: {pub_year}, 标题: {title}"
                    )
                ...
                try:
                    if impact_factor != 'N/A':
                        if isinstance (impact_factor, str):
                            impact_factor = float (
                                impact_factor.replace (',', '')
                                )
                        if float (impact_factor) >= min_if:
                            if_filtered.append (paper)
                            logger.debug (
                                f"文献 {paper.get ('pmid')} 通过影响因子筛选: {impact_factor} >= {min_if}"
                                )
                        else:
                            logger.debug (
                                f"文献 {paper.get ('pmid')} 未通过影响因子筛选: {impact_factor} < {min_if}"
                                )
                except (ValueError, TypeError) as e:
                    logger.warning (
                        f"影响因子格式错误: {impact_factor}, 错误信息: {str (e)}"
                        )
        else:
            if_filtered = year_filtered.copy ()
        stats['if_filtered'] = len (if_filtered)
        logger.info (
            f"2. 影响因子筛选 (>= {filters.get ('min_if', '无限制')}): {len (year_filtered)} -> {len (if_filtered)}"
            )
        ...
        if 'jcr_quartile' in filters and filters['jcr_quartile']:
            for paper in if_filtered:
                ...
                if jcr_q != 'N/A' and jcr_q in filters['jcr_quartile']:
                    ...
                    logger.debug (
                        f"文献 {paper.get ('pmid')} 通过JCR分区筛选: {jcr_q} in {filters['jcr_quartile']}"
                        )
                else:
                    logger.debug (
                        f"文献 {paper.get ('pmid')} 未通过JCR分区筛选: {jcr_q} not in {filters['jcr_quartile']}"
                        )
        else:
            jcr_filtered = if_filtered.copy ()
        stats['jcr_filtered'] = len (jcr_filtered)
        logger.info (
            f"3. JCR分区筛选 ({filters.get ('jcr_quartile', '无限制')}): {len (if_filtered)} -> {len (jcr_filtered)}"
            )
            ...
            for paper in jcr_filtered:
                ...
                if cas_q != 'N/A':
                    ...
                    if cas_q in cas_filters:
                        ...
                        logger.debug (
                            f"文献 {paper.get ('pmid')} 通过CAS分区筛选: {cas_q} in {cas_filters}"
                            )
                    else:
                        logger.debug (
                            f"文献 {paper.get ('pmid')} 未通过CAS分区筛选: {cas_q} not in {cas_filters}"
                            )
        else:
            cas_filtered = jcr_filtered.copy ()
        stats['cas_filtered'] = len (cas_filtered)
        logger.info (
            f"4. CAS分区筛选 ({filters.get ('cas_quartile', '无限制')}): {len (jcr_filtered)} -> {len (cas_filtered)}"
            )
        ...
        filtered_papers = sorted (
            cas_filtered,
            key=lambda x: float (
                x.get ('journal_info', {}).get ('impact_factor', '0').replace (
                    ',', ''
                    )
                )
            if isinstance (
                x.get ('journal_info', {}).get ('impact_factor'), str
                )
        ...
        if 'papers_limit' in filters and filters['papers_limit']:
            try:
                ...
                logger.info (
                    f"5. 应用数量限制 ({limit}): {len (cas_filtered)} -> {len (filtered_papers)}"
                    )
    ...
    for i, article in enumerate (soup.find_all ('pubmedarticle'), 1):
        try:
            ...
            if journal:
                ...
                journal_info[
                    'title'] = journal_title.text if journal_title else ''
            ...
            article_data = {
                'title': title,
                'abstract': article.find ('abstract').text if article.find (
                    'abstract'
                    ) else 'No abstract available',
                'authors': [
                    f"{author.find ('lastname').text} {author.find ('forename').text}"
                    for author in
                    article.find ('authorlist').find_all ('author')
                    if author.find ('lastname') and author.find (
                        'forename'
                        )] if article.find (
                    'authorlist'
                    ) else [],
                'pub_date': (lambda
                                 d: f"{d.find ('year').text if d.find ('year') else ''} {d.find ('month').text if d.find ('month') else ''}".strip ()) (
                    article.find ('pubdate')
                    ) if article.find ('pubdate') else 'Date not available',
                'pub_year': pub_year,  # 添加发表年份字段
                'pmid': article.find ('pmid').text if article.find (
                    'pmid'
                    ) else '',
                'url': f'https://pubmed.ncbi.nlm.nih.gov/{article.find ("pmid").text}/' if article.find (
                    'pmid'
                    ) else '#',
            ...
            logger.error (
                f"解析第 {i} 篇文章时发生错误: {str (e)}\n{traceback.format_exc ()}"
                )
            continue

    logger.info (
        f"完成XML解析，成功解析 {len (articles)}/{article_count} 篇文章"
        )
    return articles
...
"Coronary CT Angiography"[Title/Abstract] AND "Atherosclerotic Plaque"[Mesh]"""
        ...
        if not id_list:
            ...
            # 使用更宽泛的搜索策略
            broader_strategy = search_strategy.replace (
            ...
            if response.status_code == 200:
                search_result = response.json ()
                id_list = search_result.get ('esearchresult', {}).get (
                    'idlist', []
                    )
        ...
        if not id_list:
            ...
            basic_query = ' AND '.join (
                f'"{term}"[All Fields]' for term in basic_terms
                )
...
def fetch_paper_details (id_list):
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error (
                f"获取文献详情失败: HTTP {response.status_code}\n响应内容: {response.text}"
                )
            return []
...
def extract_paper_info (article):
    ...
    try:
        ...
        logger.info (
            f"- 完整标题: {journal_full.text if journal_full else 'N/A'}"
            )
        ...
        issn = journal_element.find ('ISSN').text if journal_element.find (
            'ISSN'
            ) else None
            ...
            if year_elem and year_elem.text:
                ...
                if medline_date and medline_date.text:
                    ...
                    try:
                        ...
                        logger.warning (
                            f"无法从MedlineDate解析年份: {medline_date.text}"
                            )
                else:
        ...
        if not journal_metrics:
            logger.warning (
                f"未找到期刊 {journal_title} (ISSN: {issn}) 的指标信息"
                )
        ...
        if abstract_element:
            abstract_text = ' '.join (
                text.text for text in abstract_element.find_all ('AbstractText')
                )
...
def process_sentence (sentence, filters=None):
    ...
    try:
        ...
        # 创建文献ID到序号的映射
        paper_index_map = {paper['pmid']: idx for idx, paper in
                           enumerate (all_papers, 1)}
...
补充策略：[使用Title/Abstract的扩展检索策略]"""
        ...
        for idx, paper in enumerate (all_papers, 1):
            formatted_output.append (
                f"{idx}. {paper.get ('title', 'No title')} (PMID: {paper['pmid']})"
                )
...
def search ():
    try:
        ...
        try:
            ...
            for i, sentence in enumerate (sentences, 1):
                try:
                    ...
                    for paper in papers:
                        ...
                        if pub_year:
                            logger.info (
                                f"文献年份提取成功 - PMID: {pmid}, 年份: {pub_year}, 标题: {title}"
                                )
                            logger.info (
                                f"期刊信息 - 影响因子: {journal_info.get ('impact_factor', 'N/A')}, JCR: {journal_info.get ('jcr_quartile', 'N/A')}, CAS: {journal_info.get ('cas_quartile', 'N/A')}"
                                )
                            papers_with_year.append (paper)
                        else:
                            logger.warning (
                                f"文献年份提取失败 - PMID: {pmid}, 标题: {title}"
                                )
                    ...
                    logger.info (
                        f"第{i}句找到{len (papers_with_year)}篇有年份信息的文献"
                        )

                    # 应用过滤条件
                    if filters:
                        logger.info (f"开始应用过滤条件: {filters}")
                        papers, stats = filter_papers_by_metrics (
                            papers_with_year, filters
                            )
                        ...
                        for paper in papers:
                            ...
                            logger.info (f"  标题: {title}")
                            logger.info (
                                f"  发表年份: {paper.get ('pub_year', 'N/A')}"
                                )
                            logger.info (
                                f"  影响因子: {journal_info.get ('impact_factor', 'N/A')}"
                                )
                            logger.info (
                                f"  JCR分区: {journal_info.get ('jcr_quartile', 'N/A')}"
                                )
                            logger.info (
                                f"  CAS分区: {journal_info.get ('cas_quartile', 'N/A')}"
                                )
                    else:
            ...
            logger.info (
                f"搜索完成，共处理{len (sentences)}句，找到结果的句子数：{len (all_results)}"
                )
            ...
            if all_results:
                ...
                if first_result['papers']:
                    ...
                    logger.info (
                        f"第一篇文献 - PMID: {first_paper.get ('pmid')}, 标题: {first_paper.get ('title', '')[:100]}"
                        )
...
if __name__ == '__main__':
    try:
        ...
        # search_strategy = call_deepseek_api ("医学规培生职业倦怠")
        # logger.info (f"生成的检索策略: {search_strategy}")
        # print ("正在启动应用服务器...")
        # 启动应用
